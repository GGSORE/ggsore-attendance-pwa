 ...
 *  - gg_headshots_map: trec_license (text unique), storage_path (text)
...
,123456-SA,First,M,Last,pay_link,,,`;
  ...
  async function clearPhoto(r: RosterRow) {
    try {
      ...
      await supabase.from("gg_headshots_map").delete().eq("trec_license", r.trec_license);
      await loadRosterFromDb(selectedSessionId);
    } catch {
...
<div style={{ display: "flex", gap: 8 }}>
  ...
  />
  <button type="button" onClick={() => setShowPw((v) => !v)} style={btnStyle("#d0efff", "#000")}>
    {showPw ? "Hide" : "Show"}
  </button>
</div>
...
<div style={{ fontSize: 12, opacity: 0.85, marginTop: 4, marginBottom: 12 }}>
  Name should appear exactly as shown on the TREC license.
</div>
...
<button type="button" onClick={signUpStudent} style={btnStyle("#1167b1")}>
  Create Account
</button>
                    </div>
                  </div>
